...
import cn.xdaoy.utils.sm.BCECUtil;
import cn.xdaoy.utils.sm.SM2Util;
...
public class SM2UtilTest extends GMBaseTest {
	...
	public void testEncryptAndDecrypt2() {
		try {
			String pubKey="040C9B6D712EC14FE492A4844E8C3EE1B590D2F453A0433D129B77F4FFEB2B03871C59EF8532B5436C140CB45C20BCC3099F8CE656F370811669CDDDE8A60EDA14";
			String priKey="0F53FE6C2924DC77E45F9B1FCC13EB639139C2B5A77C60FDEA0121C8ADFD2A90";
			String encryptedData = SM2Util.encrypt(pubKey, "中国");
			System.out.println("SM2 encrypt result:\n" + encryptedData);
	...
	public void testKeyPairEncoding() {
		try {
			...
			// FileUtils.writeFile("target/ec.pkcs8.pri.der", priKeyPkcs8Der);

			String priKeyPkcs8Pem = BCECUtil.convertECPrivateKeyPKCS8ToPEM(priKeyPkcs8Der);
			// FileUtil.writeFile("target/ec.pkcs8.pri.pem",
			// priKeyPkcs8Pem.getBytes("UTF-8"));
			...
			// FileUtil.writeFile("target/ec.x509.pub.pem",
			// pubKeyX509Pem.getBytes("UTF-8"));
	...
	public void testEncodeSM2CipherToDER_C1C2C3() {
		try {
			...
			byte[] decryptedData = SM2Util.decrypt(Mode.C1C2C3, priKey,
					SM2Util.decodeDERSM2Cipher(Mode.C1C2C3, derCipher));
			if (!Arrays.equals(decryptedData, SRC_DATA)) {
				Assert.fail();
			}
	...
	public void testGenerateBCECKeyPair() {
		try {
			...
			ECPrivateKeyParameters priKey = BCECUtil
					.convertPrivateKeyToParameters((BCECPrivateKey) keyPair.getPrivate());
