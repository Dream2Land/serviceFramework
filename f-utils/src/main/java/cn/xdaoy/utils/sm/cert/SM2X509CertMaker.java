...
public class SM2X509CertMaker {
	...
	/**
	 * @param issuerKeyPair 证书颁发者的密钥对。 其实一般的CA的私钥都是要严格保护的。
	...
	public X509Certificate makeEndEntityCert(byte[] csr, KeyPurposeId[] extendedKeyUsages) throws Exception {
		KeyUsage usage = new KeyUsage(KeyUsage.digitalSignature | KeyUsage.keyAgreement | KeyUsage.dataEncipherment
	 ...
	 * @throws Exception
	 */
	private X509Certificate makeCertificate(CertLevel certLevel, Integer pathLenConstrain, byte[] csr,
			KeyUsage keyUsage, KeyPurposeId[] extendedKeyUsages) throws Exception {
		...
		/*
		 * RFC 5280 §4.2.1.6 Subject Conforming implementations generating new
		 * certificates with electronic mail addresses MUST use the rfc822Name in the
		 * subject alternative name extension (Section 4.2.1.6) to describe such
		 * identities. Simultaneous inclusion of the emailAddress attribute in the
		 * subject distinguished name to support legacy implementations is deprecated
		 * but permitted.
		 */
		...
		X509v3CertificateBuilder v3CertGen = new X509v3CertificateBuilder(issuerDN, serialNumber, notBefore, notAfter,
				subject, subPub);
		...
		// RFC 5280 §4.2.1.3 Key Usage: When present, conforming CAs SHOULD mark this
