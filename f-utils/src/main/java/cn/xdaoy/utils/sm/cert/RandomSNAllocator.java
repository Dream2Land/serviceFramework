...
public class RandomSNAllocator implements CertSNAllocator {
	...
	/**
	 * The highest bit is always set to 1, so the effective bit length is bitLen -
	 * 1. To ensure that at least 64 bit entropy, bitLen must be at least 65.
	 */
	private final static int MIN_SERIALNUMBER_SIZE = 65;

	/**
	 * Since serial number should be positive and maximal 20 bytes, the maximal
	 * value of bitLen is 159.
	 */
	 ...
	 * 
	 * @param bitLen bit length of the serial number. The highest bit is always set
	 *               to 1, so the effective bit length is bitLen - 1. Valid value is
	 *               [65, 159].
	 */
	public RandomSNAllocator(int bitLen) {
		if (bitLen < MIN_SERIALNUMBER_SIZE || bitLen > MAX_SERIALNUMBER_SIZE) {
			throw new IllegalArgumentException(String.format("%s may not be out of the range [%d, %d]: %d", "bitLen",
