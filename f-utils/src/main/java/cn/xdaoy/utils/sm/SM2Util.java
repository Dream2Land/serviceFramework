...
public class SM2Util extends GMBaseUtil {
	...
	public static String encrypt(String pubKey, String data) throws Exception {
		return ByteUtils.toHexString(encrypt(getPubKey(pubKey), data.getBytes(StandardCharsets.UTF_8)));
	}
	...
	public static byte[] encrypt(String pubKey, byte[] data) throws Exception {
		return encrypt(getPubKey(pubKey), data);
	}
	...
	public static String decrypt(String priKey, String data) throws Exception {
		return new String(decrypt(getPriKey(priKey), ByteUtils.fromHexString(data)));
	}
	 ...
	 * @throws Exception
	 */
	public static byte[] decrypt(String priKey, byte[] data) throws Exception {
		return decrypt(getPriKey(priKey), data);
	}
	 ...
	 * @return
	 */
	public static SM2Cipher parseSM2Cipher(Mode mode, int curveLength, int digestLength, byte[] cipherText)
			throws Exception {
	 ...
	 * @return
	 */
	public static ECPrivateKeyParameters getPriKey(String priHex) {
		return new ECPrivateKeyParameters(new BigInteger(ByteUtils.fromHexString(priHex)), SM2Util.DOMAIN_PARAMS);
	}

	/**
	 * 根据pubkey hex获取public key
	 * 
	 * @param pubkey hex
	 * @return
	 */
	public static ECPublicKeyParameters getPubKey(String pubKey) {
		// remove first 04
		if (pubKey.length() > 128) {
			pubKey = pubKey.substring(2, pubKey.length());
		}
		// get x ,y
		String pubKeyx = pubKey.substring(0, 64);
		String pubKeyy = pubKey.substring(64);
		return BCECUtil.createECPublicKeyParameters(pubKeyx, pubKeyy, SM2Util.CURVE, SM2Util.DOMAIN_PARAMS);
	}
}
